...
--- @field public NAME string "ShutDown" the name of this event
local ShutDown = {
  ID = 0x0001,
  NAME = "ShutDown",
...
function ShutDown:augment_type(base_type_obj)
  local elems = {}
...
function ShutDown:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function ShutDown:subscribe(device, endpoint_id)
...
function ShutDown:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "ShutDownEventData", ID = 0x15}))
...
function ShutDown:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function ShutDown:set_parent_cluster(cluster)
