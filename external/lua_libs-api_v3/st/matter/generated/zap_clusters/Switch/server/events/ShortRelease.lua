...
--- @field public NAME string "ShortRelease" the name of this event
...
function ShortRelease:augment_type(base_type_obj)
  local elems = {}
...
function ShortRelease:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function ShortRelease:subscribe(device, endpoint_id)
...
function ShortRelease:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "ShortReleaseEventData", ID = 0x15}))
...
function ShortRelease:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function ShortRelease:set_parent_cluster(cluster)
