...
function Mode.pretty_print(value_obj)
  return string.format("%s.%s", value_obj.field_name or value_obj.NAME, Mode.enum_fields[value_obj.value])
end
...
function Mode:read(device, endpoint_id)
...
function Mode:write(device, endpoint_id, value)
...
--- @param endpoint_id number|nil
--- @return any
function Mode:subscribe(device, endpoint_id)
...
function Mode:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function Mode:deserialize(tlv_buf)
